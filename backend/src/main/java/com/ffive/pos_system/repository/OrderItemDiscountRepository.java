...
public interface OrderItemDiscountRepository extends JpaRepository<OrderItemDiscount, UUID> {
    @Modifying
    @Query("DELETE FROM OrderItemDiscount oid WHERE oid.discount = :discount AND oid.orderItem.order.status = 1")
